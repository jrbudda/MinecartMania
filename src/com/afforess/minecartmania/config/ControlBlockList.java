...
public class ControlBlockList {
	protected static HashMap<Item,ControlBlock> controlBlocks = new HashMap<Item,ControlBlock>();
	
	public static Collection<ControlBlock> getControlBlockList() {
	...
	public static double getSpeedMultiplier(MinecartManiaMinecart minecart) {
		...
		if (block != null) {
			...
			for (SpeedMultiplier speed : multipliers) {
				...
				if (speed.passenger == PassengerState.Disables && minecart.getPassenger() != null) {
					continue;
				}
				if (speed.passenger == PassengerState.Enables && minecart.getPassenger() == null) {
					continue;
				}
