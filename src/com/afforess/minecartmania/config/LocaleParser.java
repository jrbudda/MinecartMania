	...
	public boolean read(Document document) {
		...
		for (int temp = 0; temp < list.getLength(); temp++) {
			...
			if (n.getNodeType() == Node.ELEMENT_NODE) {
				...
				MinecartManiaLogger.getInstance().debug("Added Text Key Key: " + key + " Text: " + text.replace("%", "%%"));
					textKeys.put(key, text);
				}
				else {
					MinecartManiaLogger.getInstance().severe("Invalid Text Key! Key: " + key + " Text: " + text.replace("%", "%%"));
				}
			}
		}
		return true;
