...
public class CoreSettingParser implements SettingParser{
	...
	private boolean readControlBlocks(NodeList list) {
		try {
			ControlBlockList.controlBlocks = new HashMap<Item,ControlBlock>();	//init where we store the control blocks
			...
			for (int temp = 0; temp < list.getLength(); temp++) {
				...
				if (element.getNodeType() == Node.ELEMENT_NODE && element.getNodeName() == "ControlBlock") {
					...
					ControlBlockList.controlBlocks.put(cb.getType(),cb);
	...
	private void debugShowConfigs() {
		...
		Iterator<ControlBlock> li = ControlBlockList.getControlBlockList().iterator();
	...
	public boolean write(File configFile, Document document) {
		try {
			if (document == null) {
				...
				JarFile jar = new JarFile(MinecartMania.getPluginFile());
