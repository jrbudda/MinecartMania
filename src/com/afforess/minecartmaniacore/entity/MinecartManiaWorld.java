...
public class MinecartManiaWorld {
	 ...
	 */
	@ThreadSafe
	 public static MinecartManiaMinecart getOrCreateMMMinecart(Minecart minecart) {
		prune();
		...
		while (i.hasNext()) {
			...
			if (e.getValue().isDead() || e.getValue().isDead()) {
				...
				if (idList.contains(e.getValue().getEntityId())) {
					MinecartManiaLogger.getInstance().severe("Warning! Duplicate minecart's detected! Deleting duplicate. Minecart ID: " + e.getValue().getEntityId());
					i.remove();
				}
				else {
